...
export default class LanguageButton extends ButtonComponent {
  constructor() {
    super({
      classes:
        'bg-cyan-600 w-16 h-9 rounded-md font-main font-bold hover:cursor-pointer hover:bg-green-600 hover:transition-colors hover:duration-300',
    });
  ...
  private render(): void {
    ...
    this.setListeners({
      click: (): void => {
        store.dispatch({ type: CommonAppActionTypes.SWITCH_LANGUAGE });
      },
    });
  }
