...
export default function gameFetcher<State>(): Middleware<State, AppActions> {
  return function middleware(context) {
    ...
    if (context.action.type === GameActionTypes.GAME_STATE) {
      try {
        console.log('[GAME] Ожидание состояния игры');
        socketClient.onGameState(({ gameState }) => {
          console.log('[GAME] Получено состояние игры');
          context.next({ type: GameActionTypes.GAME_STATE, payload: { gameState } });
        });
      } catch (error) {
        console.error('[GAME] Ошибка GAME_STATE:', error);
        showErrorToast(error, SOCKET_ERROR_MESSAGES.ON_ERROR);
      }
    }
